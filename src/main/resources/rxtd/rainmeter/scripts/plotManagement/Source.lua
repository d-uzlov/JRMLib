-- source: https://forum.rainmeter.net/viewtopic.php?p=112978#p112978
--
...
-- v1.2.1 2017-05-01 - by rxtd - I modified this script to return max value of the graph but not the Rainmeter autoscale max
--         Also I've changed "GetOption" to "GetNumberOption" in order to be able to use formuls
--         and added error if width didn't read properly.
--
...
function Initialize()
    ...
    --sHistWidth = SELF:GetNumberOption('HistWidth', '1')
    --nHistWidth = tonumber(sHistWidth)
    nHistWidth = SELF:GetNumberOption('HistWidth', '-1')
    if (nHistWidth <= 0) then
        print('"HistWidth" option is not a valid number: HistWidth=' .. SELF:GetOption('HistWidth', '1'))
        --It should be a number more than 0.
        --If it is 0 then script will not crash but will not work correctly.
        --If it is a [Measure] then Rainmeter doesn't give it to the script at the initialization.
        --If you want to use measure as "HistWidth" then you should correct this script.
        --For example, check this parameter in the update()
    end
